...
<body>
    <div class="container">
        ...
        <div class="status-cards">
            ...
            <div class="status-card {{ isset($connection_status['success']) && $connection_status['success'] ? 'connected' : 'disconnected' }}">
                <h3>
                    <i class="fas fa-tower-broadcast"></i>
                    Gateway Connection
                </h3>
                <div class="status-value {{ isset($connection_status['success']) && $connection_status['success'] ? 'success' : 'failed' }}">
                    @if(isset($connection_status['success']) && $connection_status['success'])
                ...
                <p style="margin-top: 10px; color: #666; font-size: 13px;">
                    ...
                    {{ $connection_status['message'] ?? 'Connection status unknown' }}
                </p>
                @if(isset($connection_status['active_gateways']))
                    <div class="gateway-info">
                        <div class="gateway-info-item">
                            <span class="gateway-label"><i class="fas fa-server"></i> Active Gateways:</span>
                            <span class="gateway-value">{{ $connection_status['active_gateways'] }}</span>
                        </div>
                    </div>
                @endif
            </div>
            ...
            @if(isset($gateway_info) && isset($gateway_info['success']) && $gateway_info['success'] === true)
            <div class="status-card connected">
                ...
                <div class="status-value success">
                    ...
                    <span class="balance-amount">{{ number_format($gateway_info['current_balance'] ?? 0, 2) }}</span>
                </div>
                <div class="gateway-info">
                    <div class="gateway-info-item">
                        <span class="gateway-label"><i class="fas fa-microchip"></i> Gateway:</span>
                        <span class="gateway-value">{{ $gateway_info['gateway_name'] ?? 'N/A' }}</span>
                    </div>
                    <div class="gateway-info-item">
                        <span class="gateway-label"><i class="fas fa-signal"></i> Operator:</span>
                        <span class="gateway-value">{{ $gateway_info['operator'] ?? 'N/A' }}</span>
                    </div>
                    <div class="gateway-info-item">
                        <span class="gateway-label"><i class="fas fa-plug"></i> Connection:</span>
                        <span class="gateway-value">{{ ucfirst($gateway_info['connection_status'] ?? 'unknown') }}</span>
                    </div>
                    <div class="gateway-info-item">
                        <span class="gateway-label"><i class="fas fa-code-branch"></i> Software:</span>
                        <span class="gateway-value">{{ $gateway_info['software_active'] ? 'Active' : 'Inactive' }}</span>
                    </div>
                    @if(isset($gateway_info['supported_operators']) && count($gateway_info['supported_operators']) > 0)
                    <div class="gateway-info-item">
                        <span class="gateway-label"><i class="fas fa-mobile-alt"></i> Supports:</span>
                        <span class="gateway-value">{{ implode(', ', $gateway_info['supported_operators']) }}</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="status-card disconnected">
                ...
                <p style="margin-top: 10px; color: #666; font-size: 13px;">
                    <i class="fas fa-info-circle"></i>
                    {{ $gateway_info['message'] ?? 'Unable to fetch gateway information' }}
                </p>
            </div>
            @endif
        </div>
                        ...
                        <td>
                            @if($result['status'] == 'success')
                                <span class="transaction-id">
                                    <i class="fas fa-hashtag"></i> {{ $result['transaction_id'] ?? 'N/A' }}
                                </span>
                            @else
                                <span style="color: #dc3545; font-size: 12px;">
                                    <i class="fas fa-ban"></i> {{ $result['error'] ?? 'Unknown error' }}
                                </span>
                            @endif
                        </td>
                        <td style="max-width: 300px; word-break: break-word;">{{ $result['message'] ?? '-' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="info-box">
            <small>
                ...
                📞 <strong>Callback URL:</strong> {{ route('irecharge.callback') }}<br>
    ...
    <script>
        ...
        @if($rest_pendings > 0)
        console.log('📊 Pending loads: {{ $rest_pendings }}');
        @endif
