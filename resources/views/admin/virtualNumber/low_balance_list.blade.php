...
@section('low_balance_virtual_number_menu_class', 'active')
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 padding bg-container">
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="panel panel-danger">
                            ...
                            <div class="panel-body text-center">
                                <h2>{{ $summary['total_low_balance'] ?? 0 }}</h2>
                    ...
                    <div class="col-md-3">
                        <div class="panel panel-info">
                            ...
                            <div class="panel-body text-center">
                                <h2>{{ number_format($summary['lowest_balance'] ?? 0, 2) }} TK</h2>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="low-balance-table">
                        ...
                        <tbody>
                            @php($serial = 1)
                            @foreach($lowBalanceNumbers as $vn)
                                <tr>
                                    <td>{{ $serial++ }}</td>
                                    <td>{{ $vn['virtual_name'] }}</td>
                                    <td>{{ $vn['virtual_number'] }}</td>
                                    <td>{{ $vn['operator'] }}</td>
                                    <td>{{ $vn['username'] }}</td>
                                    <td>
                                        <strong style="color: {{ $vn['balance'] < 500 ? '#e74c3c' : '#f39c12' }}; font-size: 16px;">
                                            {{ number_format($vn['balance'], 2) }} TK
                                        </strong>
                                    </td>
                                    <td>{{ $vn['load_amount'] }}</td>
                                    <td>
                                        <span class="label label-{{ $vn['status_class'] }}">
                                            {{ $vn['status'] }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="label label-info pending-count-{{ $vn['id'] }}">Loading...</span>
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <a class="green" href="javascript:void(0)" onclick="checkBalance({{ $vn['id'] }}, '{{ addslashes($vn['virtual_name']) }}', '{{ addslashes($vn['virtual_number']) }}')" title="Check Balance">
                                                <i class="ace-icon glyphicon glyphicon-usd"></i>
                                            </a>
                                            <a class="green" href="{{ route('admin.virtualNumber.edit', $vn['id']) }}" title="Edit">
                                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                                            </a>
                                        </div>
                                    </td>
                                    <td>
                                        <button class="btn btn-warning btn-sm" onclick="showChangeSenderModal({{ $vn['id'] }}, '{{ addslashes($vn['virtual_number']) }}', '{{ addslashes($vn['operator']) }}')">
...
@section('custom_script')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            @foreach($lowBalanceNumbers as $vn)
                loadPendingCount({{ $vn['id'] }});
            @endforeach
        });
        
        function loadPendingCount(virtualNumberId) {
            $.ajax({
                url: '{{ route("admin.virtualNumber.getPendingCount") }}',
                type: 'GET',
        ...
        function showChangeSenderModal(id, virtualNumber, operator) {
            ...
            $.ajax({
                url: '{{ route("admin.virtualNumber.getSenderIds") }}',
                type: 'GET',
        ...
        function changeSenderId() {
            ...
            if (confirm('Are you sure you want to change ALL ' + pendingCount + ' pending messages from virtual number ' + virtualNumber + ' to the selected IPTSP number?\n\nThis will also change SMS type to Non-Masking (1).\n\nThis action cannot be undone!')) {
                ...
                $.ajax({
                    url: '{{ route("admin.virtualNumber.changeSenderForPending") }}',
                    type: 'POST',
                    data: {
        ...
        function refreshLowBalance() {
            ...
            $.ajax({
                url: '{{ route("admin.virtualNumber.refresh_low_balance") }}',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    $('#balance-loading').hide();
