...
<head>
    ...
    <title>{{ $filename }} - IGL SMS Log Viewer</title>
    <style>
...
<div class="wrap">
    <div class="topbar">
        <div><h1>{{ $filename }}</h1><div class="auto">Auto refresh: every 10 seconds</div></div>
        <div><a class="btn btn-back" href="{{ route('log-viewer.index') }}">Back</a> <a class="btn btn-download" href="{{ route('log-viewer.download', ['filename' => $filename]) }}">Download</a></div>
    </div>

    <form class="filters" method="GET" action="{{ route('log-viewer.show', ['filename' => $filename]) }}">
        <select name="level">
            <option value="">All levels</option>
            @foreach($levels as $item)
                <option value="{{ $item }}" {{ $level === $item ? 'selected' : '' }}>{{ strtoupper($item) }}</option>
            @endforeach
        </select>
        <input type="text" name="search" value="{{ $search }}" placeholder="Search log text">
        <button type="submit">Filter</button>
        <a class="reset" href="{{ route('log-viewer.show', ['filename' => $filename]) }}">Reset</a>
    </form>
        ...
        @foreach($entries as $entry)
            <div class="entry level-{{ $entry['level'] }}">
                <div class="entry-head">
                    <div><span class="badge badge-{{ $entry['level'] }}">{{ strtoupper($entry['level']) }}</span>&nbsp; {{ $entry['date'] }}</div>
                    <div>{{ $entry['environment'] }}</div>
                </div>
                <pre class="entry-body">{{ $entry['content'] }}</pre>
            </div>
        @endforeach
    @else
