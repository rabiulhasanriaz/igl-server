@extends('user.master')
@section('topup_menu_class', 'active')
                ...
                <div class="widget-box">
                    ...
                    <div class="widget-body">
                        <div class="widget-main">
                            <form action="{{ route('user.balance.initiate') }}" method="POST" id="topup-form">
                                @csrf
             ...
             bKash</strong> - Mobile Wallet Payment
                                ...
                                <div class="clearfix form-actions">
                                    <div class="col-md-offset-3 col-md-9">
                                        ...
                                        <a href="{{ route('user.index') }}" class="btn btn-default btn-lg" style="padding: 12px 30px; font-size: 16px;">
                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            @foreach($recentTopups as $topup)
                                                <tr>
                                                    <td>{{ $topup->asb_submit_time->format('M d, Y h:i A') }}</td>
                                                    <td>
                                                        @if($topup->asb_pay_mode == 2)
                                                            <span class="label label-info">SSLCommerz</span>
                                                        @elseif($topup->asb_pay_mode == 3)
                                                            <span class="label label-success">bKash</span>
                                                        @else
                                                            <span class="label label-default">Other</span>
                                                        @endif
                                                    </td>
                                                    <td class="text-success">+৳{{ number_format($topup->asb_credit, 2) }}</td>
                                                    <td class="text-info">
                                                        @if(isset($topup->asb_total_paid) && $topup->asb_total_paid > 0)
                                                            ৳{{ number_format($topup->asb_total_paid, 2) }}
                                                        @else
                                                            ৳{{ number_format($topup->asb_credit, 2) }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if($topup->asb_payment_status == 1 || $topup->asb_payment_status == 'completed')
                                                            <span class="label label-success">Completed</span>
                                                        @elseif($topup->asb_payment_status == 'pending')
                                                            <span class="label label-warning">Pending</span>
                                                        @elseif($topup->asb_payment_status == 'failed')
                                                            <span class="label label-danger">Failed</span>
                                                        @elseif($topup->asb_payment_status == 'cancelled')
                                                            <span class="label label-default">Cancelled</span>
                                                        @else
                                                            <span class="label label-info">{{ ucfirst($topup->asb_payment_status) }}</span>
                                                        @endif
                                                    </td>
                                                    <td><small>{{ $topup->asb_pay_ref }}</small></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
