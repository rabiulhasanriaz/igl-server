		...
		<table class="table table-striped table-bordered table-hover" id="archived-campaign-table">
			...
			@foreach($archived_campaigns as $archived_campaign)
				<tr>
					...
					<td>{{ $archived_campaign->sender->sir_sender_id ?? 'N/A' }}</td>
					...
					<td>
						<div class="btn-group">
							<a href="#my-modal" onclick="showArchivedDetails('{{ $archived_campaign->id }}')"
							   role="button" data-toggle="modal"
							   class="btn btn-xs btn-info">
								<i class="ace-icon fa fa-eye bigger-120"></i> View
							</a>
							<a href="{{ route('user.reports.download_archived_report', $archived_campaign->id) }}" 
							   target="_blank" 
...
<link rel="stylesheet" href="{{ asset('assets/datatable/jquery.dataTables.min.css') }}" type="text/css">
<link rel="stylesheet" href="{{ asset('assets/datatable/rowReorder.dataTables.min.css') }}" type="text/css">
<link rel="stylesheet" href="{{ asset('assets/datatable/responsive.dataTables.min.css') }}" type="text/css">
<style>
...
function showArchivedDetails(campaign_id) {
    ...
    $.ajax({
        url: "{{ route('ajax.show_archived_report') }}",
        type: 'GET',
        data: {
