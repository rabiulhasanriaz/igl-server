...
@section('dynamic_archived_campaign_menu_class','active')
		...
		<table class="table table-striped table-bordered table-hover">
			...
			@foreach($archived_campaigns as $archived_campaign)
				<tr>
					...
					<td>{{ $archived_campaign->sdci_targeted_time }}</td>
					<td>{{ $archived_campaign->sdci_total_submitted }}</td>
					<td>{{ $archived_campaign->sdci_total_submitted }}</td>
					<td>BDT {{ number_format($archived_campaign->sdci_total_cost, 2) }}</td>
					{{-- <td>
						<label>
