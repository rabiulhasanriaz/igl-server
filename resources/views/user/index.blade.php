...
@section('main_content')
                ...
                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="panel panel-primary dashboard-card">
                        <div class="panel-body text-center">
                            ...
                            <h2 class="stat-number">{{ number_format($data['last_week_sms'], 0) }}</h2>
                ...
                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="panel panel-green dashboard-card">
                        <div class="panel-body text-center">
                            ...
                            <h2 class="stat-number">৳ {{ number_format($data['last_week_cost'], 2) }}</h2>
                ...
                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="panel panel-orange dashboard-card">
                        <div class="panel-body text-center">
                            ...
                            <h2 class="stat-number">{{ number_format($data['last_month_sms'], 0) }}</h2>
                ...
                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="panel panel-red dashboard-card">
                        <div class="panel-body text-center">
                            ...
                            <h2 class="stat-number">৳ {{ number_format($data['last_month_cost'], 2) }}</h2>
    ...
    <div class="widget-main no-padding">
        <div class="table-responsive">
            <table class="table table-condensed table-bordered table-striped table-hover" style="margin-bottom: 0;">
                ...
                <tbody>
                    @foreach($data['monthly_sms'] as $monthly)
                    <tr>
                        <td class="text-center" style="padding: 6px 4px; text-transform: uppercase; font-size: 11px;">
                            <strong>{{ DateTime::createFromFormat('!m', $monthly->month)->format('M') }}, {{ $monthly->year }}</strong>
                        </td>
                        <td class="text-center" style="padding: 6px 4px;">
                            <span class="badge badge-success" style="font-size: 10px; padding: 3px 6px;">৳ {{ number_format($monthly->total_sms_cost, 2) }}</span>
                        </td>
                        <td class="text-center" style="padding: 6px 4px;">
                            <span class="badge badge-info" style="font-size: 10px; padding: 3px 6px;">{{ number_format($monthly->total_sms, 0) }}</span>
                        </td>
                        <td class="text-center" style="padding: 6px 4px;">
                            <span class="badge badge-warning" style="font-size: 10px; padding: 3px 6px;">
                                ৳ {{ $monthly->total_sms > 0 ? number_format($monthly->total_sms_cost / $monthly->total_sms, 4) : '0.0000' }}
                            </span>
                        </td>
                    </tr>
                    @endforeach
                ...
                <tfoot>
                    <tr class="bg-light-grey">
                        ...
                        <td class="text-center" style="padding: 6px 4px;">
                            <strong style="font-size: 11px;">৳ {{ number_format($data['monthly_sms']->sum('total_sms_cost'), 2) }}</strong>
                        </td>
                        <td class="text-center" style="padding: 6px 4px;">
                            <strong style="font-size: 11px;">{{ number_format($data['monthly_sms']->sum('total_sms'), 0) }}</strong>
                        </td>
                        <td class="text-center" style="padding: 6px 4px;">
                            <strong style="font-size: 11px;">৳ {{ $data['monthly_sms']->sum('total_sms') > 0 ? number_format($data['monthly_sms']->sum('total_sms_cost') / $data['monthly_sms']->sum('total_sms'), 4) : '0.0000' }}</strong>
                        </td>
                    </tr>
                </tfoot>
            </table>
            ...
            <li class="list-group-item" style="padding: 8px 12px; border-radius: 0; border-left: none; border-right: none;">
                <div class="clearfix">
                    <span class="pull-left">
                        ...
                        {{ \Carbon\Carbon::parse($tran->created_at)->format('M d') }}
                    </span>
                    <span class="pull-right text-success">
                        <strong>৳ {{ number_format($tran->asb_credit - $tran->asb_debit, 2) }}</strong>
                    </span>
                </div>
                <small class="text-muted" style="font-size: 11px;">
                    {{ \Carbon\Carbon::parse($tran->created_at)->format('h:i A') }}
                </small>
            </li>
            @endforeach
        </ul>
    ...
    <div class="panel-body" style="padding: 0;">
        <ul class="list-group" style="margin-bottom: 0;">
            @foreach($data['campaign_rejects'] as $camp_rej)
            <li class="list-group-item" style="padding: 6px 10px; border-radius: 0; border: none; border-bottom: 1px solid #eee;">
                <div class="clearfix">
                    <span class="pull-left" style="font-size: 12px;">
                        <i class="ace-icon fa fa-calendar"></i>
                        {{ \Carbon\Carbon::parse($camp_rej->created_at)->format('M d') }}
                    </span>
                    <span class="pull-right text-info" style="font-size: 12px;">
                        <strong>৳ {{ number_format($camp_rej->asb_credit - $camp_rej->asb_debit, 2) }}</strong>
                    </span>
                </div>
                <small class="text-muted" style="font-size: 10px;">
                    {{ \Carbon\Carbon::parse($camp_rej->created_at)->format('h:i A') }}
                </small>
            </li>
            @endforeach
        </ul>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const monthlyData = @json($data['monthly_sms']);
