<div class="table-responsive">
    @if(isset($campaign))
        <div class="well well-sm" style="margin-bottom: 15px; background-color: #f5f5f5;">
            <div class="row">
                <div class="col-md-6">
                    <strong>Campaign:</strong> {{ $campaign->sci_campaign_title ?? 'N/A' }}
                </div>
                <div class="col-md-3">
                    <strong>Campaign ID:</strong> {{ $campaign->sci_campaign_id ?? 'N/A' }}
                </div>
                <div class="col-md-3">
                    <strong>Total Records:</strong> {{ isset($reports) ? $reports->count() : 0 }}
                </div>
            </div>
        </div>
    @else
            ...
            <tbody>
                ...
                @foreach($reports as $report)
                    <tr>
                        ...
                        <td>
                            @if(isset($report->sct_cell_no))
                                {{ $report->sct_cell_no }}
                            @elseif(isset($report->sc_cell_no))
                                {{ $report->sc_cell_no }}
                            @else
                                <span class="text-muted">N/A</span>
                            @endif
                        ...
                        <td>
                            @if(isset($report->sct_message))
                                {{ substr($report->sct_message, 0, 60) }}{{ strlen($report->sct_message) > 60 ? '...' : '' }}
                            @elseif(isset($report->sc_message))
                                {{ substr($report->sc_message, 0, 60) }}{{ strlen($report->sc_message) > 60 ? '...' : '' }}
                            @else
                                <span class="text-muted">N/A</span>
                            @endif
                        </td>
                        <td>
                            @if(isset($report->sct_sms_cost))
                                {{ number_format($report->sct_sms_cost, 2) }}
                            @elseif(isset($report->sc_sms_cost))
                                {{ number_format($report->sc_sms_cost, 2) }}
                            @else
                                0.00
                            @endif
                        </td>
    ...
    @else
        <div class="alert alert-warning" style="margin: 20px 0;">
            ...
            <br><small>Campaign ID: {{ isset($campaign) ? $campaign->id : 'N/A' }}</small>
            <br><small>User ID: {{ Auth::id() }}</small>
            <br><small>Total Submitted: {{ isset($campaign) ? $campaign->sci_total_submitted : '0' }}</small>
        </div>
    @endif
</div>
