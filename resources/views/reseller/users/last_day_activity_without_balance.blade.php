...
@section('sms_activity_menu_with', 'active')
...
<h1>
    ...
    <small>
        ...
        Users who sent/didn't send SMS yesterday ({{ $yesterdayDate }} - {{ $yesterdayDayName }})
    </small>
</h1>
@endsection
...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="widget-box">
            <div class="widget-body">
                <div class="widget-main text-center bg-primary text-white" style="padding:20px;">
                    <h5>Total SMS Yesterday</h5>
                    <h2>{{ number_format($totalSmsYesterday) }}</h2>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="widget-box">
            <div class="widget-body">
                <div class="widget-main text-center bg-success text-white" style="padding:20px;">
                    <h5>Active Users</h5>
                    <h2>{{ $withSmsCount }}</h2>
                </div>
            </div>
    ...
    <div class="col-md-4">
        <div class="widget-box">
            <div class="widget-body">
                <div class="widget-main text-center bg-danger text-white" style="padding:20px;">
                    <h5>Inactive Users</h5>
                    <h2>{{ $withoutSmsCount }}</h2>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="table-responsive">
                    <table id="withSmsTable" class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="text-center">No users sent SMS yesterday</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="table-responsive">
                    <table id="withoutSmsTable" class="table table-striped table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="text-center">All users sent SMS yesterday</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
...
$(document).ready(function() {
    ...
    setTimeout(function() {
        ...
        // Initialize both tables with identical settings
        var withSmsTable = $('#withSmsTable').DataTable(dataTableOptions);
        var withoutSmsTable = $('#withoutSmsTable').DataTable(dataTableOptions);

        // Handle entries select change
        $('#entries-select').on('change', function() {
            var length = $(this).val();
            withSmsTable.page.len(length).draw();
            withoutSmsTable.page.len(length).draw();
        });

        // Tab switching and active state handling
        $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
            // Update button active states
            $('#smsFilterTabs .btn').removeClass('active');
            $(this).addClass('active');
            
            // Redraw the appropriate DataTable
            var target = $(e.target).attr("href");
            if (target === '#withSms') {
                withSmsTable.columns.adjust().responsive.recalc();
            } else {
                withoutSmsTable.columns.adjust().responsive.recalc();
            }
        });

        // Initialize first tab as active
        $('#smsFilterTabs .btn:first').tab('show');
    }, 100);
});
</script>
