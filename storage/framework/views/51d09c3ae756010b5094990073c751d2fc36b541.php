<form action="<?php echo e(route('user.sms.storeDynamicSms')); ?>" id="dynamic_sms_send" method="post" class="form-horizontal"
      role="form" enctype="multipart/form-data" style="margin-top: 10px;">
    <?php echo csrf_field(); ?>
    <div class="form-group">
        ...
        <select class="chosen-select form-control" id="form-field-select-3" data-placeholder="Select Sender ID.."
            ...
            <?php $__currentLoopData = Auth::user()->senderIds; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $senderId): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option
                    value="<?php echo e($senderId->sender->id); ?>" <?php if(old('sender_id')==true): ?> <?php echo e((@old('sender_id')==@$senderId->sender->id)? 'selected':''); ?><?php else: ?><?php echo e((@$defaultSenderId->sender_id==@$senderId->sender->id)? 'selected':''); ?><?php endif; ?>><?php echo e($senderId->sender->sir_sender_id); ?></option>
